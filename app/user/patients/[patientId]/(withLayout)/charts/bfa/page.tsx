...
const Charts = async ({params: {patientId}}) => {
  ...
  appointments?.map(appointment =>{
    if(appointment.weight && appointment.height){
      ...
      if(differenceInDays(appointment.startDate, patient?.birthdate!!) / 30.4375 > 60){
        ...
        let app = {category: differenceInDays(appointment.startDate, patient?.birthdate!!), value: parseFloat(val.toPrecision(5))}
        formatted.push(app)
      }
    }  
  })
  ...
  return (
    <>
    <Chart patient={patient} title="BMI for Age (0-5 years)"  type="bfa"  ylabel="BMI (in kg/m^2)" xlabel="Age (in days)" formatted={formatted} name={patient?.firstname} referenceData={data} />
    <Chart patient={patient} title="BMI for Age (5-19 years)"  type="bfa5To19"  ylabel="BMI (in kg/m^2)" xlabel="Age (in months)" formatted={formatted5To19} name={patient?.firstname} referenceData={data5To19} />
    </>
  )
}
