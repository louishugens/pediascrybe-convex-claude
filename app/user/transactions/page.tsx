...
export default function TransactionsPage() {
  ...
  // Initialize selectedDate on client side only
  useEffect(() => {
    if (!selectedDate) {
  ...
  useEffect(() => {
    if (!selectedDate) return // Don't fetch until date is initialized
    ...
    <div className="container mx-auto py-6 space-y-6">
      <Suspense fallback={<TransactionsPageSkeleton />}>
        ...
        <Card>
          <CardHeader>
            <CardTitle>
              Transactions for {selectedDate ? format(selectedDate, "EEEE, MMMM d, yyyy") : "..."}
            </CardTitle>
          </CardHeader>
          <CardContent>
            {loading ? (
