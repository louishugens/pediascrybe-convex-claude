...
export function ChatWindow(props: {
  ...
  const { endpoint, emptyStateComponent, placeholder, titleText = "ScrybeGPT", showIngestForm, showIntermediateStepsToggle, emoji } = props;
  ...
  return (
    <div className={`flex flex-col items-center h-full p-4 md:p-8 rounded-md bg-muted grow overflow-hidden ${(messages.length > 0 ? "border" : "")}`}>
      <h2 className={`${messages.length > 0 ? "" : "hidden"} text-2xl`}>{emoji} {titleText}</h2>
      {messages.length === 0 ? emptyStateComponent : ""}
      <div
        className="flex flex-col-reverse w-full mb-4 overflow-auto transition-[flex-grow] ease-in-out mt-4"
      ...
      <form onSubmit={sendMessage} className="flex w-full flex-col">
        ...
        <div className="flex w-full mt-4">
          <input
            className="grow mr-8 px-4 py-2 rounded-full border border-primary w-full"
            value={input}
            placeholder={placeholder ?? "What's the name of this patient?"}
            onChange={handleInputChange}
          />
          <button type="submit" className="shrink-0 px-8 py-2 text-white bg-primary rounded-full w-28">
            <div role="status" className={`${(chatEndpointIsLoading || intermediateStepsLoading) ? "" : "hidden"} flex justify-center`}>
              ...
              <BeatLoader size={6} color={'#fff'} loading={chatEndpointIsLoading || intermediateStepsLoading} />
            </div>
